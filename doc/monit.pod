...
be disabled to prevent possibly restarting a compromised binary:
 ...
 check process apache with pidfile /var/run/httpd.pid
       ...
       stop program  = "/etc/init.d/httpd stop"
       if failed host www.example.com port 80 then restart
...
No rights to this software are granted except for those provided to you by Metaswitch Networks in a separate written agreement or for third party components as described in the Clearwater Core Open Source Acknowledgements document.
